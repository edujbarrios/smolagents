...
> [!IMPORTANT]
> **This is a community fork of [huggingface/smolagents](https://github.com/huggingface/smolagents)** maintained by [@edujbarrios](https://github.com/edujbarrios).
> It tracks the upstream library and adds several extra features (see [Fork Add-ons](#fork-add-ons) below).
>
> Because the fork ships **development versions** (e.g. `1.25.0.dev0`) that are **not published to PyPI**, you must install directly from source:
>
> ```bash
...
> ```
>
> If you only want the runtime (no dev/test tooling):
>
> ```bash
...
The following features are **exclusive to this fork** and are not (yet) part of upstream `smolagents`.
